...
import com.yunbao.game.bean.BankerBean;
...
import com.yunbao.game.dialog.GameNzLsDialogFragment;
import com.yunbao.game.dialog.GameNzSfDialogFragment;
import com.yunbao.game.dialog.GameNzSzDialogFragment;
...
public class GameNzViewHolder extends AbsGameViewHolder {
    ...
    public void showBankerInfo() {
        ...
        if (mBankerAvatar != null) {
            if ("0".equals(mBankerBean.getBankerId()) || TextUtils.isEmpty(mBankerBean.getBankerAvatar())) {
                ...
                ImgLoader.displayAvatar(mContext,mBankerBean.getBankerAvatar(), mBankerAvatar);
            }
        }
        if (mBankerName != null) {
    ...
    public void anchorCreateGame() {
        ...
        GameHttpUtil.gameNiuzaiCreate(mStream, mBankerBean.getBankerId(), new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                if (code == 0) {
                    ...
                    if (mGameActionListener != null) {
                        mGameActionListener.nzAnchorCreateGame(mGameID, obj.getString("bankerlist"));
                    }
                } else {
                    ToastUtil.show(msg);
                }
    ...
    private void playCardAnim() {
        ...
        if (mAnchor && mGameActionListener != null) {
            mGameActionListener.nzAnchorNotifyGameBet(mLiveUid, mGameID, mGameToken, mBetTime);
        }
    }
    ...
    private void audienceBetGame(final int index) {
        GameHttpUtil.gameNiuzaiBet(mGameID, mBetMoney, index, new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                if (code == 0 && info.length > 0) {
                    setLastCoin(JSON.parseObject(info[0]).getString("coin"));
                    if (mGameActionListener != null) {
                        mGameActionListener.nzAudienceBetGame(mBetMoney, index);
                    }
                } else {
                    ToastUtil.show(msg);
                }
    ...
    public void anchorCloseGame() {
        ...
        if (mGameActionListener != null) {
            mGameActionListener.nzAnchorCloseGame();
        }
    ...
    public void release() {
        ...
        GameHttpUtil.cancel(GameHttpConsts.GAME_NIUZAI_CREATE);
        GameHttpUtil.cancel(GameHttpConsts.GAME_NIUZAI_BET);
    ...
    public void onClick(View v) {
        ...
        } else if (i == R.id.group_water) {
            showWater();

        } else if (i == R.id.btn_sz) {
            showSz();
    ...
    private void showSz() {
        ...
        mGameNzSzDialogFragment.show(((AbsActivity) mContext).getSupportFragmentManager(), "GameNzSzDialogFragment");
    }
    ...
    private void showWater() {
        ...
        mGameNzLsDialogFragment.show(((AbsActivity) mContext).getSupportFragmentManager(), "GameNzLsDialogFragment");
    }
    ...
    private void showRecord() {
        ...
        mGameNzSfDialogFragment.show(((AbsActivity) mContext).getSupportFragmentManager(), "GameNzSfDialogFragment");
    }
}
