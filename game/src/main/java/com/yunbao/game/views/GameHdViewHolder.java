...
public class GameHdViewHolder extends AbsGameViewHolder {
    ...
    public void anchorCreateGame() {
        ...
        GameHttpUtil.gameHaidaoCreate(mStream, new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                if (code == 0) {
                    ...
                    if (mGameActionListener != null) {
                        mGameActionListener.hdAnchorCreateGame(mGameID);
                    }
                } else {
                    ToastUtil.show(msg);
                }
    ...
    private void playCardAnim() {
        ...
        if (mAnchor && mGameActionListener != null) {
            mGameActionListener.hdAnchorNotifyGameBet(mLiveUid, mGameID, mGameToken, mBetTime);
        }
    }
    ...
    private void audienceBetGame(final int index) {
        GameHttpUtil.gameHaidaoBet(mGameID, mBetMoney, index, new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                if (code == 0 && info.length > 0) {
                    setLastCoin(JSON.parseObject(info[0]).getString("coin"));
                    if (mGameActionListener != null) {
                        mGameActionListener.hdAudienceBetGame(mBetMoney, index);
                    }
                } else {
                    ToastUtil.show(msg);
                }
    ...
    public void anchorCloseGame() {
        ...
        if (mGameActionListener != null) {
            mGameActionListener.hdAnchorCloseGame();
        }
    ...
    public void release() {
        ...
        GameHttpUtil.cancel(GameHttpConsts.GAME_HAIDAO_CREATE);
        GameHttpUtil.cancel(GameHttpConsts.GAME_HAIDAO_BET);
