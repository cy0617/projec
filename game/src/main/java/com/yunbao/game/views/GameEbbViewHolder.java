...
import com.yunbao.game.custom.GameEbbView;
...
public class GameEbbViewHolder extends AbsGameViewHolder {
    ...
    public void anchorCreateGame() {
        ...
        GameHttpUtil.gameEbbCreate(mStream, new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                if (code == 0) {
                    ...
                    if (mGameActionListener != null) {
                        mGameActionListener.ebbAnchorCreateGame(mGameID);
                    }
                } else {
                    ToastUtil.show(msg);
                }
    ...
    private void playCardAnim() {
        ...
        if (mAnchor && mGameActionListener != null) {
            mGameActionListener.ebbAnchorNotifyGameBet(mLiveUid, mGameID, mGameToken, mBetTime);
        }
    }
    ...
    private void audienceBetGame(final int index) {
        GameHttpUtil.gameEbbBet(mGameID, mBetMoney, index, new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                if (code == 0 && info.length > 0) {
                    setLastCoin(JSON.parseObject(info[0]).getString("coin"));
                    if (mGameActionListener != null) {
                        mGameActionListener.ebbAudienceBetGame(mBetMoney, index);
                    }
                } else {
                    ToastUtil.show(msg);
                }
    ...
    public void anchorCloseGame() {
        ...
        if(mGameActionListener!=null){
            mGameActionListener.ebbAnchorCloseGame();
        }
    ...
    public void release() {
        ...
        GameHttpUtil.cancel(GameHttpConsts.GAME_EBB_CREATE);
        GameHttpUtil.cancel(GameHttpConsts.GAME_EBB_BET);
