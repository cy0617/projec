...
public class LiveShareAdapter extends RecyclerView.Adapter<LiveShareAdapter.Vh> {
    ...
    public LiveShareAdapter(Context context,boolean mDelete,boolean noLink) {
        mList = new ArrayList<>();
...
//        }
        if(mDelete){
            MobBean linkBean = new MobBean();
            linkBean.setType(Constants.DELETE);
            linkBean.setName(R.string.delete);
            linkBean.setIcon1(R.mipmap.icon_share_video_delete);
            mList.add(linkBean);
        }
