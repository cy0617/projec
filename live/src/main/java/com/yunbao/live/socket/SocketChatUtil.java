...
public class SocketChatUtil {
     ...
     * @param client
     */
    public static void sendShopExplainMsg(GoodsBean goodsBean,SocketClient client) {
        if (client == null) {
            return;
        }
        UserBean u = CommonAppConfig.getInstance().getUserBean();
        ...
        client.send(new SocketSendBean()
                .param("_method_", Constants.SOCKET_SHOP_TALKING)
                .param("action", 0)
                .param("msgtype", 0)
                .param("uid", u.getId())
                .param("uname", u.getUserNiceName())
                .param("id", goodsBean.getId())
                .param("thumb", goodsBean.getThumb())
                .param("price", goodsBean.getPriceNow())
                .param("istalking", goodsBean.getIstalking())
        );
    }
