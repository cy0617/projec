...
public class UserHomeSharePresenter {
     ...
     * 分享页面链接
     */
    public void shareUserRecordPage(String type,String recordId) {
        if (TextUtils.isEmpty(mToUid)) {
            return;
        }
        ...
        ShareData data = new ShareData();
        data.setTitle(configBean.getLiveback_share_title());
        data.setDes(configBean.getLiveback_share_des());
        data.setImgUrl(mToAvatarThumb);
        String webUrl = configBean.getLiveback_siteurl() + recordId;
        data.setWebUrl(webUrl);
        mMobShareUtil.execute(type, data, null);
    }
