...
import com.yunbao.live.event.LiveRecordDeleteEvent;
...
//    private ImageView mLevelAnchor;
    ...
    public static void forward(Context context, String url, UserBean userBean, String recordId) {
    ...
    protected void main() {
        ...
        mRecordId = intent.getStringExtra(Constants.RECORD_ID);
...
//        }
    ...
    private void share() {
        ...
        if (mUserBean!=null){
            fragment.setHasDelete(mUserBean.getId().equals(CommonAppConfig.getInstance().getUid()));
        }
    ...
    public void onItemClick(String type) {
        if (Constants.DELETE.equals(type)) {
            delete();
        } else {
            shareHomePage(type);
        }
     ...
     */
    private void delete() {
        LiveHttpUtil.delLiveRecord(mRecordId, new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                ToastUtil.show(msg);
                if (code == 0) {
                    EventBus.getDefault().post(new LiveRecordDeleteEvent());
                    finish();
                }
            }
        });
    ...
    private void shareHomePage(String type) {
        if (mUserHomeSharePresenter != null) {
            mUserHomeSharePresenter.shareUserRecordPage(type,mRecordId);
        }
    }
